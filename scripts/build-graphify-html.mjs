...
(function enhanceAtlas() {
  ...
  function nodeVisualFor(node, baseColor, degree, isCommand, isHub) {
    ...
    return {
      ...
      pulseOffset: stableHash(node.id) % 2200,
  ...
  function drawRoundNode(ctx, visual, scale) {
    ...
    if (visual.pulse && pulseEnabled) {
      ...
      const phase = ((pulseTime + visual.pulseOffset) % pulseDuration) / pulseDuration;
      const secondaryPhase = (phase + 0.46) % 1;
      const alphaBase = visual.tier === 'core' ? 0.46 : visual.tier === 'hub' ? 0.36 : 0.28;
      drawPulseRing(ctx, visual, scale, ring, phase, alphaBase, 4.4, 18, 1.24, 0.54);
      if (visual.tier === 'core' || visual.tier === 'hub') {
        drawPulseRing(ctx, visual, scale, ring, secondaryPhase, alphaBase * 0.52, 8.8, 22, 0.84, 0.34);
      }
    }
  ...
  function drawPulseRing(ctx, visual, scale, ring, phase, alphaBase, startGap, spread, startWidth, endWidth) {
    const pulseScale = Math.max(scale, 0.64);
    const eased = 1 - Math.pow(1 - phase, 2);
    const radius = ring + (startGap + eased * spread) / pulseScale;
    const alpha = Math.max(0, alphaBase * Math.pow(1 - phase, 1.28));
    ctx.lineWidth = (startWidth - phase * (startWidth - endWidth)) / pulseScale;
    ctx.strokeStyle = rgbaFromHex(visual.pulseColor, alpha);
    ctx.beginPath();
