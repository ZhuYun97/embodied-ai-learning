...
检索过去 **7 天**内具身智能(VLA / WAM / 人形机器人 / 通用机器人基础模型)领域的真实**非论文新闻事件**,排除已收录事件(下方「已收录指纹」)。

本站现在执行内容分流:
- **论文、综述、arXiv 预印本、论文型 benchmark / dataset paper** → 进入 `/papers/latest`,不要输出到新闻 JSON
- **公司战略、产品发布、融资上市、产业落地、活动竞赛、政策标准、非论文型开源/模型/平台发布** → 进入本新闻页
...
6. **论文分流优先级高于新闻优先级**:
   - 纯 arXiv 论文、综述、论文型 benchmark、论文型 dataset release 不输出到 `news`
   - 如果一篇论文同时伴随真实非论文事件(例如官方模型权重/产品平台上线、公司级发布会、开源工具被正式发布),新闻项只写“发布事件”,不要写成论文摘要
   - 论文线索可在调研备注里保留,但最终 JSON 中不要出现 `category: ["论文发表"]`
7. **优先一手与权威源**:
   - 一手:官方博客(deepmind.google / openai.com / pi.website / nvidia.com 等)、公司公告、交易所/会议/活动官网
...
8. **同一事件在多源出现时,引用最权威的那个**(优先官方,其次顶级媒体)

## 8 路并行检索方向

1. **非论文模型/产品/平台发布**:官方模型权重、代码库、机器人产品、平台能力上线;纯 arXiv/论文介绍不要报到新闻页
...
5. **数据集与评测基准的非论文事件**:官方数据/benchmark 平台、榜单、竞赛系统上线;若只是论文投稿或预印本,转给 `/papers/latest`
...
- B 轮+ 融资
- 明星机构产品发布
- 重要数据/benchmark 平台或榜单发布(非纯论文)

**📌 normal 关注**:
- 一般产品迭代、活动动态、非论文型开源更新
- A 轮 / 天使轮 融资
- 普通产品迭代
- 中小媒体报道
- 次要会议
