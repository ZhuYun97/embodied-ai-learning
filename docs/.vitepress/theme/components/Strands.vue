...
const props = defineProps({
  ...
  refraction: { type: Number, default: 1 },
  dispersion: { type: Number, default: 1 },
  glassSize: { type: Number, default: 1 },
})
...
function buildPalette(colors, Color) {
  const filled = Array.isArray(colors) && colors.length ? colors : ['#ffffff']
  const padded = []
...
onMounted(async () => {
  ...
  const uniforms = {
    ...
    uColors: { value: buildPalette(props.colors, Color) },
    uColorCount: { value: Math.min((props.colors || []).length, MAX_COLORS) },
    uStrandCount: { value: clamp(Math.round(props.count), 1, MAX_STRANDS) },
  ...
  const resize = () => {
    const width = Math.max(1, Math.round(container.offsetWidth || 1))
    const height = Math.max(1, Math.round(container.offsetHeight || 1))
  ...
  const syncUniforms = (t) => {
    ...
    if (colorsKey !== paletteKey) {
      paletteKey = colorsKey
      program.uniforms.uColors.value = buildPalette(props.colors, Color)
      program.uniforms.uColorCount.value = Math.min((props.colors || []).length, MAX_COLORS)
    }

    program.uniforms.uTime.value = t * 0.001
    program.uniforms.uStrandCount.value = clamp(Math.round(props.count), 1, MAX_STRANDS)
  ...
  const start = () => {
    if (!raf && !destroyed && visible) raf = requestAnimationFrame(renderFrame)
  }
  const stop = () => {
...
.strands-container.is-static {
  background:
    linear-gradient(115deg, transparent 0 18%, rgba(56, 189, 248, 0.2) 34%, transparent 52%),
    linear-gradient(65deg, transparent 0 44%, rgba(246, 198, 103, 0.14) 58%, transparent 76%);
  opacity: 0.58;
}
</style>
