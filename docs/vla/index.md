...
[![GPT-IMAGE2 生成的 VLA 分类图:从感知、语言推理到动作控制,下分离散 token、连续动作、混合双系统、世界模型与从经验强化学习五条路线](/figures/vla-taxonomy-gpt-image2.png)](/figures/vla-taxonomy-gpt-image2.png)

*读图方式:上层是 VLA 的基本闭环——感知 → 语言推理 → 动作控制;下层把当前主流工作按动作/学习范式拆成五类:离散 token、连续动作、混合/双系统、世界模型、从真机经验中强化学习。*
...
**VLA 谱系总览**(线 = 技术路线 · 站 = 论文细读,点击进站;横轴为 arXiv 提交年月。WAM 主线的对应谱系图见 [WAM 总览](/wam/)):
...
| 具身数据论文索引 | 数据集 / 人类视频 / 合成数据 / 筛选 / 基准 | 把站内外具身数据相关论文按用途聚合,作为进入数据方向的目录页 | [→ 索引](papers/embodied-data-papers.md) |
