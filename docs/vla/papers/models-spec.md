...
> **三处权威源声明(务必先读)**:① **规格以各模型细读为权威源**(本表的数字均可在对应 `xx.md` 里找到出处);② **成绩(成功率/基准分)一律见 [评测基准](benchmarks.md)**,本表不抄成绩;③ **年代序与机构以 [发展时间线](timeline.md) 为准**。
...
| **SmolVLA** | 2025.06 | Hugging Face / LeRobot | SmolVLM-2(SigLIP + SmolLM2,冻结;只用前 16 层 LLM) | SigLIP(64 token/帧,pixel shuffle,只全局图) | ~0.45B(VLM 冻结 + ~100M 流匹配动作专家;另有 0.24B/2.25B) | 流匹配(交错 CA/SA 动作专家,动作块 n=50,10 步去噪) | 异步推理栈;30fps 控制周期 Δt=33ms ⚠️ | 481 社区数据集筛出 22.9K episode / 10.6M 帧(Qwen2.5-VL-3B 清洗标注) | 单体(冻结 VLM + 动作专家) | **完全开源**(代码/权重/数据,Apache-2.0,集成 LeRobot) | [smolvla.md](smolvla.md) |
...
> **关于"成绩"**:上表**刻意不列任何成功率/基准分**。各模型的 LIBERO / SimplerEnv / 真机成功率及其可信度标注,**统一见 [评测基准](benchmarks.md)**(那里区分了 ⚠️ 自评与 ✅ 第三方核查)。
...
2. **成绩见 [benchmarks.md](benchmarks.md)。** 本表不列任何成功率/基准分;所有定量成绩及其 ⚠️/✅ 可信度标注,统一以 [评测基准](benchmarks.md) 为准。
