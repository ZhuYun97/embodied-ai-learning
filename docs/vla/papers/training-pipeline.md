...
[![GPT-IMAGE2 生成的具身模型训练流水线图:从数据湖到预训练、协同训练、后训练、真机经验和部署反馈](/figures/training-pipeline-cover-gpt-image2.jpg)](/figures/training-pipeline-cover-gpt-image2.jpg)

*读图方式:训练不是单次 fit,而是「语义预训练 → 机器人动作注入 → 协同训练 → 后训练/RL → 部署反馈」逐层叠能力。*
