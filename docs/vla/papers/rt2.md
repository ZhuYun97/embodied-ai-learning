...
- **综合成功率**:在所有类别上 VLA 模型显著优于基线,其中最好的 **RT-2-PaLI-X 的平均成功率比次优基线 RT-1 高 3 倍以上**。

  | 对比对象 | 相对 RT-2 的表现(论文口径) |
  |---|---|
  | RT-1 / MOO(泛化评测,次优两基线) | RT-2 约为其 **~2x** |
  | 其他基线(泛化评测) | RT-2 约为其 **~6x** |
  | RT-1(整体平均成功率,次优基线) | RT-2-PaLI-X **>3x** |
