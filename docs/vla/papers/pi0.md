...
π0 原论文的核心评测(Figure 7–13)以**真机成功率柱状图**呈现,论文正文未给逐任务数值表;下表把"原论文质性结论"与"后续可比基准上的定量数字"合并为一张速览,**严格区分口径与来源**。读表第一原则:不同来源/口径的数字**不可直接横比**(详见 [基准专题](benchmarks.md))。

**(a) 论文自身真机评测(Physical Intelligence 自评,质性为主)**

| 模型 / 设定 | 任务 | 指标 | 数值 | 来源 / 标注 |
|---|---|---|---|---|
| π0(700k 步,完整预训练) | 叠衬衫 / 收餐桌 / 装袋 / 取吐司等灵巧任务 | 真机成功率(柱状图) | **显著高于全部基线** | Figure 7,质性 ⚠️ 作者自评 |
| π0(160k 步,算力对齐) | 同上 | 真机成功率 | 仍 **胜过全部基线** | Figure 7,质性 ⚠️ 作者自评 |
| OpenVLA(7B) | 同上 | 真机成功率 | 明显落后 π0 | Figure 7,质性 ⚠️ 基线 |
| Octo(93M) | 同上 | 真机成功率 | 明显落后 π0 | Figure 7,质性 ⚠️ 基线 |
| π0-small(470M,无 VLM 初始化) | 同上 | 真机成功率 | 不及主模型,但 **仍胜 OpenVLA / Octo** | §2.5 / 第4节,质性 ⚠️ 作者自评 |
| π0(完整预训练) | 叠衣服 / 收午餐桌 / 组装纸盒 / 装鸡蛋 / 打包外卖盒等长程多阶段 | 各任务平均归一化得分 | **平均 > 满分的 50%**,通常优于各消融 | Figure 10–13,质性 ⚠️ 作者自评 |

**(b) 后续可比基准上的 π0 定量数字(口径各异,均带 ⚠️)**

| 模型 / 设定 | 基准 · 口径 | 指标 | 数值 | 来源 / 标注 |
|---|---|---|---|---|
| π0-Beta | SimplerEnv · Google Robot,Visual Matching | 成功率 % | **71.4** | ⚠️ PI 口径;数取自 **MemoryVLA 作者评测**一栏,非 PI 原作者直接挂出,**无严格第三方复现**(benchmarks.md §1.5) |
| π0-Beta | SimplerEnv · WidowX/Bridge,Visual Matching | 成功率 % | **68.4** | ⚠️ PI 口径;同上,缺严格复现 |
| π0-Uniform | SimplerEnv · WidowX/Bridge,Visual Matching | 成功率 % | 55.7 | ⚠️ PI 口径(benchmarks.md §1.4) |
| π0(openpi) | RoboCasa 1.0 multitask · 300 任务 / 100 demos / pretrain scenes | Avg 成功率 % | **14.8**(Atomic-Seen 34.6 / Composite-Seen 6.1 / Composite-Unseen 1.1) | ✅ 由 RoboCasa 官方维护方统一训练评测(非厂商自评);似为按任务加权均(benchmarks.md §4.3) |
| π0(re-evaluated) | CALVIN · ABC→D 零样本 | avg-len /5 | 3.509(T1–T5: 0.896/0.785/0.786/0.610/0.532) | ⚠️ 第三方 VLM4VLA 重评,**移除本体感知专家、仅单图输入**,口径有改动;PI 官方未评 CALVIN(benchmarks.md §3.5) |

> **同口径排名参照(RoboCasa 1.0 multitask,唯一对 π0 严格同口径的可比表)**:Diffusion Policy 6.1% < **π0 14.8%** < π0.5 16.9% < GR00T N1.5 20.0%。
> ⚠️ **口径提醒**:(a) 为论文真机自评(无统一公开数表),(b) 中 SimplerEnv 71.4/68.4 为厂商口径且缺严格复现;RoboCasa 14.8% 虽由维护方统一评测可信度高,但与论文原始真机任务**不是同一套评测**。OpenVLA / Octo 在 SimplerEnv/RoboCasa 的对照见 [基准专题](benchmarks.md)(此处不混算)。
...
相关条目:[[pi0-fast]] · [[pi05]] · [[octo]] · [[openvla]]
