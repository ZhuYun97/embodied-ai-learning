...
[![GPT-IMAGE2 生成的 VLA 失败模式诊断图:机器人任务中的定位、遮挡、接触、长程误差和延迟问题被仪表盘式聚合](/figures/failure-modes-cover-gpt-image2.jpg)](/figures/failure-modes-cover-gpt-image2.jpg)

*读图方式:失败不是一个点,而是感知、语言、动作、接触、时序和系统延迟共同累积;本页把微观逐帧错误和宏观成功率断崖接起来看。*
