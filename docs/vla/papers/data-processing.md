---
description: 具身数据处理深度调研(VLA × WAM 通用):从原始遥操作/视频流到可训练样本的全流程工程——数据清洗与质检、语言/子目标/奖励标注、动作处理(归一化/分词/分块/跨本体对齐)、观测处理、伪标签生成(IDM/潜动作/手部关键点/合成真值)、数据配比采样与格式工具链(RLDS/LeRobot/HDF5/Zarr)。
title: 具身数据处理深度调研
---

# 具身数据处理深度调研:从原始采集到可训练样本

> 本篇是 VLA 与 WAM 共用的「具身数据处理」基础专题,聚焦**拿到原始数据之后怎么把它处理成可训练样本**的算法与工程细节。
> [← 返回主报告](../index.md)
