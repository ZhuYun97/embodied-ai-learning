---
description: 梳理具身智能实验所用机器人本体(VLA × WAM 通用),从单臂 WidowX、Franka/DROID 到双臂 ALOHA 与人形 GR-1、Unitree G1、Apollo,讲清各平台形态、自由度与跨本体迁移,涵盖 RT-1、Octo、OpenVLA、π0、GR00T 等模型的本体绑定。
title: 实验机器人本体梳理
---

# 实验机器人本体梳理:具身实验都跑在哪些机器人上

> [← 返回主报告](../index.md)

> **定位**:本篇是 VLA 与 WAM 共用的「实验机器人本体」基础专题,聚焦具身实验**跑在哪些机器人硬件上**——平台形态、自由度、谁在用、如何跨本体迁移,而非数据(见 [具身数据](embodied-data.md))或基准(见 [评测基准](benchmarks.md))。
...
这套归一化是 co-training 的技术前提,详见 [具身数据 §6.3](embodied-data.md) 与 [评测基准](benchmarks.md)。
...
*本篇为《VLA 发展深度调研报告》「实验机器人本体」专题子文档,基于语料关联 + 厂商规格调研综合而成。⚠️ 标记处为厂商/提出方自评,「待核」处规格未独立调研。互链:[具身数据](embodied-data.md) · [评测基准](benchmarks.md)。*
