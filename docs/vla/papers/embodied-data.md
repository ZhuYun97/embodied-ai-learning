---
description: 具身智能数据专题(VLA × WAM 通用):从真机遥操作轨迹、互联网视觉-语言数据到人类视频与仿真合成数据,梳理潜动作、逆动力学、跨本体归一化与 co-training,以及 NVIDIA GR00T 数据金字塔与 π0.5 异构协同训练范式。
title: 具身数据全景
---

# 具身数据全景梳理:从真机轨迹到数据金字塔

> **定位**:本篇是 VLA 与 WAM 两条主线**共用**的「具身数据」基础专题,聚焦具身智能的**数据**一侧——数据从哪来、怎么采、怎么配、怎么 scale,而非模型架构本身(VLA 架构见 [VLA 调研报告](../index.md),WAM 见 [WAM 总览](../../wam/index.md))。
